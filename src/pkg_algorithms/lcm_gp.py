...
class DfsDataGP:
    ...
    def fit_tids_v2(self, file_path):
        d_gp = DataGP(file_path)
        d_gp.fit_bitmap()
        gi_to_tids = defaultdict(set)
        for bin_obj in d_gp.valid_bins:
            arr_ij = np.transpose(np.nonzero(bin_obj[1].T))  # Remember to remove .T
            set_ij = {tuple(ij) for ij in arr_ij if ij[0] < ij[1]}
            int_gi = int(bin_obj[0][0]+1) if (bin_obj[0][1].decode() == '+') else (-1 * int(bin_obj[0][0]+1))
            if len(set_ij) > 0:
                gi_to_tids[int_gi] = set_ij
        gi_to_tids = SortedDict(gi_to_tids)
        print(gi_to_tids)
    ...
    def _encode_data(self):
        ...
        print(encoded_data)
        encoded_data = self._remove_invalid_attrs(np.array(encoded_data), cost_matrix)
...
d_obj = DfsDataGP(filePath, 0.2)
d_obj.fit_tids_v2(filePath)
